...
PROP_CLIENT_API_USERNAME = 'api_user_uuid'
PROP_CLIENT_API_PASSWORD = 'api_password'
...
def _rest_client_retry_and_auth(f):
    ...
    def wrapper(self, endpoint, data=None, **kwargs):
        ...
        url = '{service_url}/rest/user/{api_version}/{endpoint}'.format(
            ...
            endpoint=endpoint)
        p_url = '/rest/user/{api_version}/{endpoint}'.format(
            service_url=self.service_url, api_version=REST_API_VERSION,
            endpoint=endpoint)
        ...
        while retry_count:
            ...
            self.logger.info('%s %s', f.__name__.upper(), p_url)
